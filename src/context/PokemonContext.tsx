...
const PokemonProvider = ({ children }: any) => {
  ...
  const changeTypeSelected = async (type: PokeType) => {
    ...
    const pokemons = data?.pokemon?.map(({ pokemon }: PokemonsByTypeResult) => pokemon?.url);

    type.name !== 'All' ? setPokemonsFiltered(pokemons) : setPokemonsFiltered(allPokemons);
  };
  ...
  const getAllPokemons = async () => {
    ...
    const pokemons = data?.results?.map((pokemon: AllPokemonsResult) => pokemon?.url);
